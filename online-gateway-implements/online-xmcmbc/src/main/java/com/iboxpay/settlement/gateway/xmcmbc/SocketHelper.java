...
public class SocketHelper {
	...
	public Socket getSocket() throws FrontEndException {
		canRun = Boolean.parseBoolean((String) configParams.get("CAN_RUN"));
		if(!canRun) {
			reset();
			return null;
		}
	...
	private Socket initSocket() throws FrontEndException {
		String ip = (String) configParams.get("CONNECT_HOST_IP");// "172.30.4.92"
		int port = Integer.parseInt((String) configParams.get("CONNECT_HOST_PORT"));// 9010
		try {
			...
			logger.error("民生本行打开网络连接失败" + "(ip:" + ip + ", port:" + port + ")", e);
			reset();
			
//			throw new FrontEndException("民生本行打开网络连接失败" + "(ip:" + ip + ", port:" + port + ")", e);
		}
	...
	private void checkConnect() {
		...
		try {
			...
			if (ss > heartInterval) {//30s发一次心跳包
				logger.info("民生本行Socket最后活跃时间:" + DateTimeUtil.format(this.getLastActiveTime(), "yyyy-MM-dd HH:mm:ss"));
				...
				if (socket != null) {
					...
					output.flush();
					logger.info("民生本行心跳包" + heartbeatMessage + "发送成功");
				}
			}
		} catch (Exception e) {
			reset();
	...
	private void reset() {// 重置，关闭，清空
		...
		try {
			...
			logger.error("民生本行关闭socket发生异常:" + e.getMessage());
		} finally {
			socket = null;// 清空socket
		}
	}
	...
	private void sendMessage() throws Exception {
		...
		try {
			Socket socket = getSocket();
			if (socket != null) {
				logger.info("民生本行开始发送报文");
				OutputStream os = socket.getOutputStream();
				os.write(bytes);
				os.flush();
				this.setLastActiveTime(new Date());
				logger.info("民生本行结束发送报文");
			} else {
				logger.info("民生本行getSocket is null");
				throw new NullPointerException("socket == null");
			}
		} catch (Exception e) {
			logger.error("民生本行发送报文出现异常\n:{}", e);
			//			this.close();
			try {
				if (tranId != null) {
		...
		try {
			...
			if (socket != null) {
				...
				if (in.available() > 0) {
					...
					byte[] bodyBytes = ArrayUtils.subarray(bytes, headLength + 283, headLength + bodyLength);
					//logger.info("民生本行接收【支付】返回报文bodyBytes: \n" + new String(bodyBytes, charset));
					// 对密文解密
					...
					if (StringUtils.isNotBlank(tranId)) {
						synchronized (keyMap.get(tranId)) {
							SocketHelper.rspMap.put(tranId, decryptedStr);
							SocketHelper.countDownMap.get(tranId).countDown();
						}
					}
				}
				else {
    ...
    final static class SystemManagerListener extends Thread {
        ...
        private static void runManager() throws IOException {
        	...
        	int port = Integer.parseInt(reader.getPropertyValue("/xmcmbc.properties", "CAN_RUN_PORT"));

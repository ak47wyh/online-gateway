...
@Service
public class RefundPaymentDelegateService implements ITransDelegate,RunnableTask{
	private final Logger logger = LoggerFactory.getLogger(RefundPaymentDelegateService.class);
	...
	public CommonResultModel trans(TransContext context, CommonRequestModel requestModel) {
	    context.setTransCode(TransCode.REFUND);
        ...
        if(paymentQueryList!=null&&paymentQueryList.size()>0){
        	for (PaymentEntity paymentEntity : paymentQueryList) {
				...
				}else if(paymentEntity.getStatus()==PaymentStatus.STATUS_SUCCESS 
					   ||paymentEntity.getStatus()== PaymentStatus.STATUS_SUBMITTED
					   ||paymentEntity.getStatus()== PaymentStatus.STATUS_UNKNOWN){
					paymentList.add(paymentEntity);
				}
			}
        }else {
        	return resultModel.fail(ErrorCode.payment_not_exist, "交易记录不存在");
        }
        ...
        for (PaymentEntity paymentEntity : paymentList) {
        	...
        	for(int i=0;i<data.length;i++){
        		...
        		if(rs.getSeqId().equals(paymentEntity.getSeqId().trim())){
        			...
        			if(!extPropertiesMap.containsKey("outRefundNo")){
        				extPropertiesMap.put("outRefundNo", DateTimeUtil.getTimeMillisY(new Date(), "yyyyMMddHHmmss") );
        			}
        ...
        if (paymentList.size() == 1) {//原来也只有一笔
            params = new HashMap<String, Object>();
            batchNum.set(1);
            params.put(PARAM_RPAYMENT, paymentList.toArray(new PaymentEntity[0]));
            params.put(BATCH_NUM, batchNum);
            return TaskScheduler.scheduleTask(TransCode.REFUND, context.getMainAccount(), context.getBankProfile().getBankName(), params, false);//同步返回
        } else {//异步处理
                //key:银行批次，value:数据库ID. 按银行批次划分后查询
            Map<String, List<Long>> bankBatchIdMap = new HashMap<String, List<Long>>();
            for (PaymentEntity needReqBankPayment : paymentList) {
            ...
            paymentService.cache(paymentList);//放到cache中，在后台调度时不用再到数据库取
            while (itr.hasNext()) {
                ...
                params.put(BATCH_NUM, batchNum);
                TaskScheduler.scheduleTask(TransCode.REFUND, context.getMainAccount(), context.getBankProfile().getBankName(), params, "q:" + model.getBatchSeqId() + "," + entry.getKey(), true);
            }
        }
		return resultModel;
	}
	...
	public CommonResultModel run(TaskParam taskParam) {
	 ...
	 private void doRefund(TransContext context, PaymentEntity[] payments,AccountEntity accountEntity) {
	    ...
	    Class<? extends IRefundPayment> queryClass = paymentImpl.getRefundClass();
	    IRefundPayment refundPaymentImpl = (IRefundPayment) BankTransComponentManager.getBankComponent(queryClass); 
		 
		try {
			refundPaymentImpl.refund(payments);
		} catch (Throwable e) {

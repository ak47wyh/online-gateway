...
import com.iboxpay.settlement.gateway.common.trans.refund.RefundPaymentDelegateService;
import com.iboxpay.settlement.gateway.common.util.JsonUtil;

@Service
public class ClosePaymentDelegateService implements ITransDelegate,RunnableTask{
	private final Logger logger = LoggerFactory.getLogger(ClosePaymentDelegateService.class);
	...
	public CommonResultModel trans(TransContext context, CommonRequestModel requestModel) {
		System.out.println("关闭组件测试");
	    context.setTransCode(TransCode.CLOSE);
        ...
        List<PaymentEntity> paymentEntitys = paymentDao.findByHQL(
							        		"from PaymentEntity where batchSeqId=? and (status=? or status=?) order by id asc", 
							        		batchPaymentEntity.getBatchSeqId(),
							        		PaymentStatus.STATUS_SUBMITTED,
        ...
        if (paymentEntitys.size() == 1) {//原来也只有一笔
            ...
            return TaskScheduler.scheduleTask(TransCode.CLOSE, context.getMainAccount(), context.getBankProfile().getBankName(), params, false);//同步返回
        } else {//异步处理
            ...
            while (itr.hasNext()) {
                ...
                params.put(BATCH_NUM, batchNum);
                TaskScheduler.scheduleTask(TransCode.CLOSE, context.getMainAccount(), context.getBankProfile().getBankName(), params, "q:" + model.getBatchSeqId() + "," + entry.getKey(), true);
            }
        }
		return resultModel;
	}
	...
	public CommonResultModel run(TaskParam taskParam) {
	 ...
	 private void doRefund(TransContext context, PaymentEntity[] payments,AccountEntity accountEntity) {
	    ...
	    Class<? extends IClosePayment> queryClass = paymentImpl.getCloseClass();
	    IClosePayment refundPaymentImpl = (IClosePayment) BankTransComponentManager.getBankComponent(queryClass); 
		 
		try {
			refundPaymentImpl.closeOrder(payments);
		} catch (Throwable e) {
			String message = "关闭订单结果后更新数据库失败";
